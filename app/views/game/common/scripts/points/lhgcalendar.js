...
var calendar = function(b) {
    ...
    p.applnk(J.path() + '../../../common/style/points/' + 'lhgcalendar.css');
...
J.exend(calendar.prototype, {
    ...
    createbody: function(d, m) {
        ...
        txt;
        txt = '<div id="date"><span id="year"></span><p class="cal_month"><span id="month"></span>月</p><span id="day"></span></div>';
        txt = '<h3>星期二</h3><div id="py">&nbsp;</div><div id="pm">&nbsp;</div>' + txt + '<div id="ny">&nbsp;</div><div id="nm">&nbsp;</div>';
        J(chead).stcs({
            width: '100%',
            height: 'auto'
        }).html(txt);
        J(m).apch(chead);
        txt = '<table width="100%" cellspacing="0" cellpadding="0" border="0">' + '<thead id="tdy"><tr><td>日</td><td>一</td><td>二</td><td>三</td><td>四</td><td>五</td><td>六</td></tr></thead>' + '<tbody id="idcal"></tbody></table>' + '<div class="btnPop"><button class="btnS_o btn_cancle">取消</button><button class="btnS_g">确定</button></div>';
